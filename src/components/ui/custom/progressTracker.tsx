...
export default function BookingProgressTracker({
  ...
  return (
    <Card className="mb-5 w-full border border-gray-500 px-2 shadow-sm md:p-6">
      {/* Order Header */}
      <CardHeader>
      ...
      <CardContent className="px-0 pb-3 text-center sm:px-6 sm:pb-6">
        <div className="flex flex-col gap-1 text-sm sm:flex-row sm:items-center sm:justify-center sm:gap-4">
          <span className="text-sm font-semibold md:text-lg">
            ${servicePrice}
          </span>
          {placedDate && (
            <span className="text-xs sm:text-sm">Placed: {placedDate}</span>
          )}
        </div>
        {expectedCompletion && (
      ...
      <CardFooter className="flex-col gap-3 px-0 sm:gap-4 sm:px-6">
        {/* Progress Tracker */}
        <div className="w-full space-y-3 sm:space-y-4">
          ...
          <div className="text-center">
            <div
              className={`
              inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-medium sm:px-4 sm:text-sm
              ${
